...
local t = {
	...
	},
	{
		...
		---@return boolean
		function (pPlayer)
			return SirinLua.ButtonMgr.loadScripts()
		end
	},
	{
		...
		---@return boolean
		function (pPlayer)
			return SirinLua.PotionMgr.loadScripts()
		end
	},
	{
		"bot add", "111100", "111",
	...
	},
	{
		...
		---@return boolean
		function (pPlayer)
			Sirin.mainThread.modRadar.addSign(pPlayer, 1, 20, pPlayer.m_pCurMap.m_nMapCode, pPlayer.m_fCurPos_x, pPlayer.m_fCurPos_z, 4, 5, 39, true, "Demo text")
			return true
		end
	},
	{
		"unset dot", "111111", "111",
		---@param pPlayer CPlayer
		---@return boolean
		function (pPlayer)
			Sirin.mainThread.modRadar.removeSign(pPlayer, math.floor(tonumber(Sirin.mainThread.getCheatWord(0)) or 0), math.floor(tonumber(Sirin.mainThread.getCheatWord(1)) or 0))
			return true
		end
	},
	{
		"add dot type", "111111", "111",
		---@param pPlayer CPlayer
		---@return boolean
		function (pPlayer)
			local send = {}
			...
			d.type = 1
			d.icon = { 4, 5, 35 }
			d.description = "custom type 1"
			d.clickable = true
			table.insert(send.data, d)
			d = {}
			d.type = 2
			d.icon = { 4, 5, 36 }
			d.description = "custom type 2"
			d.clickable = false
			table.insert(send.data, d)
			local netOP = NetOP:new()
			netOP:SendData(pPlayer, "sirin.proto.customMinimap_TypeAdd", send, true)
			return true
		end,
	},
	{
		...
		function (pPlayer)
			...
			d.id = 100
			d.type = 1
			d.mapIndex = pPlayer.m_pCurMap.m_pMapSet.m_dwIndex
			d.x = pPlayer.m_fCurPos_x
			d.y = pPlayer.m_fCurPos_z
			table.insert(send.data, d)
			d = {}
			d.id = 101
			d.type = 2
			d.mapIndex = pPlayer.m_pCurMap.m_pMapSet.m_dwIndex
			d.x = pPlayer.m_fCurPos_x + 500
			d.y = pPlayer.m_fCurPos_z + 500
			table.insert(send.data, d)
			d = {}
			d.id = 102
			d.type = 1
			...
			table.insert(send.data, d)
			local netOP = NetOP:new()
			netOP:SendData(pPlayer, "sirin.proto.customMinimap_PointAdd", send, true)
			return true
		end,
	},
	{
		...
		function (pPlayer)
			local send = {}
			send.mode = math.floor(tonumber(Sirin.mainThread.getCheatWord(0)) or 0)
			send.data = {}
			local n = math.floor(tonumber(Sirin.mainThread.getCheatWordNum()) or 0)
			local i = 1

			repeat
				local num = math.floor(tonumber(Sirin.mainThread.getCheatWord(i)) or 0)
				print(num, " ", i, " ", n)
				table.insert(send.data, num)
				i = i + 1
			until i == n

			local netOP = NetOP:new()
			netOP:SendData(pPlayer, "sirin.proto.customMinimap_PointRemove", send, true)
			return true
		end,
	},
}
