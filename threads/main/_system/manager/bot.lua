...
function sirinBotMgr.init()
	...
	for ri,r in ipairs(presetsArmor) do
		sirinBotMgr.presetsArmor[ri] = {}

		for ci,c in ipairs(r) do
			sirinBotMgr.presetsArmor[ri][ci] = {}

			for si,s in ipairs(c) do
				sirinBotMgr.presetsArmor[ri][ci][si] = {-1, -1, -1, -1, -1}
				...
				for _,a in ipairs(s) do
					...
					if tblCode == -1 then
						...
						if not pItemFld then
							...
							sirinBotMgr.presetsArmor[ri][ci][si][tblCode + 1] = pItemFld.m_dwIndex
						end
					end
				end
			end
	...
	for ci,c in ipairs(presetsWeapon) do
		sirinBotMgr.presetsWeapon[ci] = {}

		for si,s in ipairs(c) do
			sirinBotMgr.presetsWeapon[ci][si] = {}
			...
			for _,a in ipairs(s) do
				...
				if tblCode == -1 then
					...
					if not pItemFld then
						...
						table.insert(sirinBotMgr.presetsWeapon[ci][si], pItemFld.m_dwIndex)
					end
				end
			end
		end
...
function sirinBotMgr.onLoop()
	...
	repeat
		...
		local bot = sirinBotMgr.bots[sirinBotMgr.loopIndex]
	...
	DWORD dwSerial;
			...
			if time - bot.actionStart > bot.actionTime then
				...
				elseif raction <= 40 then -- action move (1)
					...
					local pRet = 0
					pRet, newPos[2] = sirinBotMgr.player.m_pCurMap.m_Level:GetNextYposForServerFar(bot.pos[1], bot.pos[2], bot.pos[3], newPos[1], newPos[2], newPos[3])
...
struct _player_move_zocl
{
	...
	BYTE byDirect;
					...
					buf:PushInt16(5)
					buf:PushUInt8(0)
					buf:SendBuffer(sirinBotMgr.player, 4, 4)
					bot.pos = newPos
...
struct _attack_skill_result_zocl
{
	...
	_attack_gen_result_zocl::_dam_list DamList[32];
					...
					if bot.byClass == 2 and bot.byRaceSexCode ~= 4 then -- attack force
						...
						buf:PushUInt8(0)
						buf:PushUInt16(0)
						buf:SendBuffer(sirinBotMgr.player, 5, 9)
					else -- attack skill
						buf:PushUInt8(0)
						buf:PushUInt32(bot.objSerial)
						...
						buf:PushUInt8(0)
						buf:PushUInt16(0)
						buf:SendBuffer(sirinBotMgr.player, 5, 8)
					end
				end
			end
		end
...
---@param nNum integer
function sirinBotMgr.makeBot(pPlayer, nNum)
	...
	for i = 1, nNum do
		...
		for j = 0, 1 do
			local bot = {}
			bot.live = false
			bot.index = sirinBotMgr.startIndex
			sirinBotMgr.startIndex = sirinBotMgr.startIndex + 1
			bot.objSerial = sirinBotMgr.startSerial
			sirinBotMgr.startSerial = sirinBotMgr.startSerial + 1
			...
			local equipPreset = sirinBotMgr.presetsArmor[math.floor(bot.byRaceSexCode / 2) + 1][bot.byClass + 1]
			local weaponPreset = sirinBotMgr.presetsWeapon[bot.byClass + 1][bot.byClass == 2 and bot.byRaceSexCode == 4 and 2 or 1]
			...
			table.insert(sirinBotMgr.bots, bot)
...
function sirinBotMgr.initHooks()
	SirinLua.HookMgr.addHook("CNetworkEX__OtherShapeRequest", HOOK_POS.special, sirinBotMgr.m_strUUID, sirinBotMgr.otherShapeRequest)
end
