---@class (exact) sirinPotionMgr
---@field __index table
---@field m_pszModPotionEffectLogPath string
---@field m_Handlers table<integer, fun(pActChar: CCharacter, pTargetChar: CCharacter, effect: number|table): boolean, integer>
---@field m_Registered table<integer, integer>
---@field loadScripts fun(): boolean
---@field uninit fun()
---@field defaultHandler fun(pActChar: CCharacter, pTargetChar: CCharacter, fEffectValue: number): boolean, integer
local sirinPotionMgr = {
	m_pszModPotionEffectLogPath = '.\\sirin-log\\guard\\ModPotionEffect.log',
	m_Handlers = {
		---1. Add cash handler
...
function sirinPotionMgr.uninit()
	for _,v in ipairs(sirinPotionMgr.m_Registered) do
		Sirin.mainThread.modPotionEffect.removeHandler(v)
	end
end

---@return boolean
function sirinPotionMgr.loadScripts()
	local bSucc = true

	repeat
		SirinTmp_PotionEffect = FileLoader.LoadChunkedTable(".\\sirin-lua\\threads\\main\\ReloadableScripts\\PotionEffect")

		if not SirinTmp_PotionEffect then
			Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, "Failed to load 'PotionEffect' scripts!\n")
			Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, "Failed to load 'PotionEffect' scripts!\n", true, true)
			bSucc = false
			break
		end
		...
		for k,v in pairs(SirinTmp_PotionEffect) do
			repeat
				local pFld = Sirin.mainThread.g_Main:m_tblItemData_get(TBL_CODE.potion):GetRecordByHash(k, 2, 5)

				if not pFld then
					local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s invalid potion code!\n", k)
					Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
					Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
					bSucc = false
					break
				end
				...
				elseif type(v) == "table" then
					if type(v[1]) ~= "number" then
						local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[1] invalid type! Number expected.\n", k)
						Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
						Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
						bSucc = false
						break
					...
					if v[1] < 1 or v[1]> #sirinPotionMgr.m_Handlers then
						local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[1] out of range!\n", k)
						Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
						Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
						bSucc = false
						break
					...
					if v[1] == 4 then
						if type(v[2]) ~= "table" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2] invalid type! Table expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
						...
						if type(v[2][1]) ~= "string" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][1] invalid type! String expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
						...
						for i = 2, 5 do
							if type(v[2][i]) ~= "boolean" then
								local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][%d] invalid type! Boolean expected.\n", k, i)
								Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
								Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
								bSucc = false
								break
						...
						if not Sirin.mainThread.g_Main.m_tblMonster:GetRecord(v[2][1]) then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2] invalid monster code: (%s)!\n", k, v[2][1])
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
					...
					elseif v[1] <= 8 then
						if type(v[2]) ~= "number" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2] invalid type! Number expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
					...
					elseif v[1] == 9 then
						...
						if type(v[2][1]) ~= "number" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][1] invalid type! Number expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
						...
						if type(v[2][2]) ~= "boolean" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][2] invalid type! Boolean expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
					...
					elseif v[1] == 10 then
						...
						if type(v[2][3]) ~= "boolean" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][3] invalid type! Boolean expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
					...
					elseif v[1] <= 14 then
						...
						if type(v[2][3]) ~= "number" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][3] invalid type! Number expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
						...
						if v[2][4] and type(v[2][4]) ~= "number" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][4] invalid type! Number expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
						...
						if not pEffFld then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][1] invalid effect code: (%s)!\n", k, v[2][1])
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
						...
						if v[2][3] < 1 or v[2][3] > 7 then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][3] out of range!\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
					...
					elseif v[1] == 15 then
						...
						if type(v[2][2]) ~= "number" then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][2] invalid type! Number expected.\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
						...
						if v[2][1] < 1 or v[2][1] > 8 then
							local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s[2][1] out of range!\n", k)
							Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
							Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
							bSucc = false
							break
					...
					local fmt = string.format("Lua. sirinPotionMgr.loadScripts() Potion record:%s invalid type! Function or table expected.\n", k)
					Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, fmt)
					Sirin.WriteA(sirinPotionMgr.m_pszModPotionEffectLogPath, fmt, true, true)
					bSucc = false
					break
		...
		if bSucc then
			SirinScript_PotionEffect = newScriptData
			SirinTmp_PotionEffect = nil

			sirinPotionMgr.uninit()
			sirinPotionMgr.m_Registered = {}

			for k,v in pairs(SirinScript_PotionEffect) do
				Sirin.mainThread.modPotionEffect.addHandler(k)
				table.insert(sirinPotionMgr.m_Registered, k)
			end
		else
...
function sirinPotionMgr.defaultHandler(pActChar, pTargetChar, fEffectValue)
	local byRet = 0
	local bSucc = false

	repeat
		if not SirinScript_PotionEffect then
			byRet = 100
			break
		end

		local effect = SirinScript_PotionEffect[math.floor(fEffectValue)]
		...
		if type(effect) == "function" then
			...
			bSucc, byRet = sirinPotionMgr.m_Handlers[effID](pActChar, pTargetChar, effect[2])
		end
	until true
