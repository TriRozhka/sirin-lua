...
---@param pAT CAttack
function sirinAnimus.calcAttExpForPlayer(pAnimus, pAT)
				...
				if pPlayer.m_pPartyMgr:IsPartyMode() then
						...
						if fPartyExp >= 1.0 then
							...
							if (SERVER_AOP and bExpForMaster) or not SERVER_AOP then
								if v:IsRidingUnit() then
									...
									v:Emb_AlterStat(MTY_CODE.special, 0, math.floor(fPartyExp / 180 + nDstLv), 0, "sirinAnimus.calcAttExpForPlayer() -- 1", true)
								else
									v:AlterExp(fPartyExp, false, false, false)
								end
							end
					...
					if SERVER_AOP and bExpForMaster then
						if pPlayer:IsRidingUnit() then
							...
							pPlayer:Emb_AlterStat(MTY_CODE.special, 0, math.floor(fExp / 180 + nDstLv), 0, "sirinAnimus.calcAttExpForPlayer() -- 2", true)
						else
							pPlayer:AlterExp(fExp, false, false, false)
						end
					end
