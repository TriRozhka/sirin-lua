...
function sirinCharacter.assistSkill(pSrcCharacter, pDstCharacter, nEffectCode, pSkillFld, nLv, bOverride, dwDurationOverride)
	if bOverride then
		local deleteIndex = sirinCharacter.getAppliedEffectIndex(pDstCharacter, nEffectCode, pSkillFld.m_dwIndex)
...
---@return integer byErrCode
---@return boolean bUpMastery
function sirinCharacter.assistForce(pSrcCharacter, pDstCharacter, pForceFld, nLv, bOverride, dwDurationOverride)
	if bOverride then
		local deleteIndex = sirinCharacter.getAppliedEffectIndex(pDstCharacter, EFF_CODE.force, pForceFld.m_dwIndex)
...
---@return integer
function sirinCharacter.getAppliedEffectIndex(pCharacter, nEffectCode, dwScriptIndex)
	...
	repeat
		...
		if Sirin.mainThread.modContEffect.isUse() and pCharacter.m_ObjID.m_byID == ID_CHAR.player and nEffectClass ~= 4 then
			...
			for i = 0, 7 do
				repeat
					if not pCharacter:m_SFCont_get(nContType, i).m_bExist then
						break
					end

					if pCharacter:m_SFCont_get(nContType, i).m_byEffectCode ~= nEffectCode then
						break
					end

					if pCharacter:m_SFCont_get(nContType, i).m_wEffectIndex ~= pFld.m_dwIndex then
						break
					end
...
---@return integer
function sirinCharacter.CCharacter__GetAttackDamPoint(pAttacker, nAttPnt, nAttPart, nTolType, pDst, bBackAttack)
	local fDefPnt = 0.0
