...
function SirinDummyRift:Close()
	...
	self.m_bOpen = false
	RiftMgr:removeRift(self.m_pCObj)
end
...
function SirinDummyRift:Open(pParam)
	...
	repeat
		if not self.m_pCObj:Create(pParam) then
			Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Lua SirinDummyRift:Open(pParam) Rift:'%s' CGameObject::Create() Failed!\n", self.m_strScriptID), true, true)
			break
		end
	...
	if bRet then
		RiftMgr:addRift(self.m_pCObj, self)
	...
	if self.m_LuaScript.buttonName then
		for i = 0, 2531 do
			repeat
				...
				local strPrefix = MainThread.getPlayerLanguagePrefix(i) or 'default'
				local strButtonName = ""
...
function SirinDummyRift:SendMsg_FixPosition(nIndex)
	...
	if self.m_LuaScript.buttonName then
		...
		local strPrefix = MainThread.getPlayerLanguagePrefix(nIndex) or 'default'
		local strButtonName = ""
		local strDescription = ""
...
function SirinDummyRift:openExitDummy(bUse)
	...
	if not self.m_ExitDummyGate:IsOpen() then
		...
		if not self.m_ExitDummyGate:Open(param) then
			if RiftMgr:IsDebugLog() then
				print(string.format("Rift '%s' exit dummy open fail", self.m_strScriptID))
				Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Rift '%s' exit dummy open fail\n", self.m_strScriptID), true, true)
			end
...
function SirinDummyRift:openEnterDummy()
	...
	if not self.m_pSrcMap then
		Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Lua. SirinDummyRift:openEnterDummy() Rift:'%s' self.m_pSrcMap == NULL\n", self.m_strScriptID), true, true)
		return
	end
	...
	if not self:Open(param) then
		if RiftMgr:IsDebugLog() then
			...
			Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Rift '%s' open fail\n", self.m_strScriptID), true, true)
		end
		...
		if RiftMgr:IsDebugLog() then
			...
			Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Rift '%s' open success. Next close: %s\n", self.m_strScriptID, szBufr), true, true)
		end
...
function SirinRift:Close()
	...
	if RiftMgr:IsSaveState() then
		...
		if RiftMgr:IsDebugLog() then
			...
			Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Rift '%s' closed. Next open: %s\n", self.m_strScriptID, szBufr), true, true)
		end

		Sirin.WritePrivateProfileStringA(self.m_strScriptID, "IsOpen", "0", RiftMgr.pszModReturnGateConfigPath)
...
function SirinRift:Open(pParam)
	...
	repeat
		if not self.m_pDstMap then
			Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Lua. SirinRift:Open(pParam) Rift:'%s' self.m_pDstMap == NULL\n", self.m_strScriptID), true, true)
			break
		end
		...
		if MapFld.m_nMapType ~= 0 then
			if not self.m_pCObj.m_pDestMap:m_ls_get(self.m_DstPos[4]):IsActiveLayer() then
				...
				if nErr ~= 0 then
					Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Lua. SirinRift:Open(pParam) Rift:'%s' Dst.activateLayer(%d, %d, true) != E_OK\n", self.m_strScriptID, MapFld.m_dwIndex, self.m_DstPos[4]), true, true)
					break
				end
			end
		end
		...
		if MapFld.m_nMapType ~= 0 then
			if not pParam.m_pMap:m_ls_get(pParam.m_nLayerIndex):IsActiveLayer() then
				...
				if nErr ~= 0 then
					Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Lua. SirinRift:Open(pParam) Rift:'%s' Src.activateLayer(%d, %d, true) != E_OK\n", self.m_strScriptID, MapFld.m_dwIndex, pParam.m_nLayerIndex), true, true)
					break
				end
			end
		end
		...
		if not self.m_pCObj:Create(pParam) then
			Sirin.WriteA(RiftMgr.pszModReturnGateLogPath, string.format("Lua. SirinRift:Open(pParam) Rift:'%s' CGameObject::Create() Failed!\n", self.m_strScriptID), true, true)
			break
		end
		...
		if not self.m_bReOpen then
			...
			Sirin.WritePrivateProfileStringA(self.m_strScriptID, "IsOpen", "1", RiftMgr.pszModReturnGateConfigPath)
			Sirin.WritePrivateProfileStringA(self.m_strScriptID, "UseLeft", string.format("%d", self.m_nUseLeft), RiftMgr.pszModReturnGateConfigPath)
			Sirin.WritePrivateProfileStringA(self.m_strScriptID, "CloseTime", string.format("%d", self.m_tNextCloseTime), RiftMgr.pszModReturnGateConfigPath)
		end
...
---@field pszModReturnGateLogPath string
---@field pszModReturnGateConfigPath string
local SirinRiftMgr = {
	m_bSaveState = true,
	m_tLoopTime = os.time(),
	m_Gates = {},
	...
	pszModReturnGateLogPath = '.\\sirin-log\\guard\\ModReturnGate.log',
	pszModReturnGateConfigPath = '..\\SystemSave\\ModReturnGate.ini',
}
...
---@return SirinRiftMgr self
function SirinRiftMgr:new(o)
	o = o or {}
	self.__index = self
	return setmetatable(o, self)
end

RiftMgr = SirinRiftMgr:new()

---@return boolean m_bDebugLog
function SirinRiftMgr:IsDebugLog()
	return self.m_bDebugLog
end

---@param bSet boolean
function SirinRiftMgr:EnableSaveState(bSet)
	self.m_bSaveState = bSet
end

---@return boolean m_bSaveState
function SirinRiftMgr:IsSaveState()
	return self.m_bSaveState
end

---@param pRift CDummyRift
---@return SirinDummyRift
function SirinRiftMgr:getRift(pRift)
	return self.m_ListGates[pRift.m_strObjectUUID]
end

---@param pRift CDummyRift
---@param pLuaRift SirinDummyRift
function SirinRiftMgr:addRift(pRift, pLuaRift)
	self.m_ListGates[pRift.m_strObjectUUID] = pLuaRift
end

---@param pRift CDummyRift
function SirinRiftMgr:removeRift(pRift)
	self.m_ListGates[pRift.m_strObjectUUID] = nil
end
...
function SirinRiftMgr:loadScripts()
	...
	repeat
		TmpDynamicPortals = FileLoader.LoadChunkedTable(".\\sirin-lua\\Rifts")

		if not TmpDynamicPortals then
			Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, "Failed to load 'Rift' scripts!\n")
			Sirin.WriteA(self.pszModReturnGateLogPath, "Failed to load 'Rift' scripts!\n", true, true)
			break
		end
...
function SirinRiftMgr:validateScriptData()
	...
	repeat
		local luaScript = TmpDynamicPortals
		...
		for strCode,portal_data in pairs(luaScript) do
			repeat
				if type(portal_data) ~= 'table' then
					bSucc = false
					Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s invalid format!\n", strCode), true, true)
					break
				end

				if type(portal_data.portalType) ~= 'number' then
					bSucc = false
					Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s portalType invalid format!\n", strCode), true, true)
					break
				end

				if portal_data.portalType ~= 0 then
					bSucc = false
					Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s portalType out of range!\n", strCode), true, true)
					break
				end
				...
				for _,t in ipairs(pszMapType) do
					repeat
						...
						if not portal_data[pszBufr] or type(portal_data[pszBufr]) ~= 'string' then
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s %s invalid!\n", strCode, pszBufr), true, true)
							bSucc = false
							break
						end
						...
						if not pMap then
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s %s (%s) not found!\n", strCode, pszBufr, strMapName), true, true)
							bSucc = false
							break
						end
						...
						if not portal_data[pszBufr] or type(portal_data[pszBufr]) ~= 'table' or #portal_data[pszBufr] == 0 then
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s %s invalid format!\n", strCode, pszBufr), true, true)
							bSucc = false
							break
						end
						...
						for dwIndex,pos in pairs(portal_data[pszBufr]) do
							repeat
								if not pos or type(pos) ~= 'table' or #pos < 4 then
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s %s[%d] invalid format!\n", strCode, pszBufr, dwIndex), true, true)
									bSucc = false
									break
								end
								...
								if not pMap:IsMapIn(pos[1], pos[2], pos[3]) then
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s %s[%d] not in Map range!\n", strCode, pszBufr, dwIndex), true, true)
									bSucc = false
									break
								end

								if pMap.m_Level:GetNextYposForServer(pos[1], pos[2], pos[3]) ~= 1 then
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s %s[%d] there is no valid Y coordinate for this position!\n", strCode, pszBufr, dwIndex), true, true)
									bSucc = false
									break
								end
								...
								if MapFld.m_nMapType ~= 0 and pos[4] >= MapFld.m_nLayerNum then
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s layer id (%d) >= Max layer (%d) !\n", strCode, pos[4], MapFld.m_nLayerNum), true, true)
									bSucc = false
									break
								end

								if MapFld.m_nMapType == 0 and pos[4] ~= 0 then
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s layer id (%d) != 0 (%d) !\n", strCode, pos[4], MapFld.m_nLayerNum), true, true)
									bSucc = false
									break
								end
				...
				if portal_data.exitGateType then
					if type(portal_data.exitGateType) ~= 'number' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s exitGateType invalid format!\n", strCode), true, true)
						break
					end

					if portal_data.exitGateType < 0 or portal_data.exitGateType > 2 then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s exitGateType out of range!\n", strCode), true, true)
						break
					end
				end
				...
				if portal_data.exitGateDelay then
					if type(portal_data.exitGateDelay) ~= 'number' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s exitGateDelay invalid format!\n", strCode), true, true)
						break
					end

					if portal_data.exitGateDelay < 0 then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s exitGateDelay out of range!\n", strCode), true, true)
						break
					end
				end
				...
				if portal_data.probability then
					if type(portal_data.probability) ~= 'number' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s probability invalid format!\n", strCode), true, true)
						break
					end

					if portal_data.probability == 0 then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s probability out of range!\n", strCode), true, true)
						break
					end
				end
				...
				if portal_data.useCount then
					if type(portal_data.useCount) ~= 'number' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s useCount invalid format!\n", strCode), true, true)
						break
					end

					if portal_data.useCount == 0 then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s useCount out of range!\n", strCode), true, true)
						break
					end
				end
				...
				if portal_data.buttonName then
					if type(portal_data.buttonName) ~= 'table' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s buttonName invalid format!\n", strCode), true, true)
						break
					end

					if not portal_data.buttonName.default then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s buttonName.default invalid format!\n", strCode), true, true)
						break
					end
				end
				...
				if portal_data.description then
					if type(portal_data.description) ~= 'table' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s description invalid format!\n", strCode), true, true)
						break
					end

					if not portal_data.description.default then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s description.default invalid format!\n", strCode), true, true)
						break
					end
				end
				...
				if portal_data.onOpen and type(portal_data.onOpen) ~= 'function' then
					bSucc = false
					Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s description.onOpen invalid format!\n", strCode), true, true)
					break
				end

				if portal_data.onClose and type(portal_data.onClose) ~= 'function' then
					bSucc = false
					Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s description.onClose invalid format!\n", strCode), true, true)
					break
				end

				if type(portal_data.onCheckUseConditions) ~= 'function' then
					bSucc = false
					Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s description.onCheckUseConditions invalid format!\n", strCode), true, true)
					break
				end

				if type(portal_data.onUse) ~= 'function' then
					bSucc = false
					Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s description.onUse invalid format!\n", strCode), true, true)
					break
				end
				...
				if portal_data.conditions then
					if type(portal_data.conditions) ~= 'table' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions invalid format!\n", strCode), true, true)
						break
					end
					...
					if c.minLevel and type(c.minLevel) ~= 'number' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.minLevel invalid format!\n", strCode), true, true)
						break
					end

					if c.maxLevel and type(c.maxLevel) ~= 'number' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.maxLevel invalid format!\n", strCode), true, true)
						break
					end
					...
					if c.pvpGradeLimit then
						if type(c.pvpGradeLimit) ~= 'number' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.pvpGradeLimit invalid format!\n", strCode), true, true)
							break
						end

						if c.pvpGradeLimit > 7 then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.pvpGradeLimit out of range!\n", strCode), true, true)
							break
						end
					end
					...
					if c.patriarchGroupLimit then
						if type(c.patriarchGroupLimit) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.patriarchGroupLimit invalid format!\n", strCode), true, true)
							break
						end
						...
						for dwIndex,v in ipairs(c.patriarchGroupLimit) do
							if type(v) ~= 'number' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.patriarchGroupLimit[%d] invalid format!\n", strCode, dwIndex), true, true)
								break
							end

							if v > 9 or v < -9 then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.patriarchGroupLimit[%d] out of range!\n", strCode, dwIndex), true, true)
								break
							end
						end
					...
					if c.raceLimit then
						if type(c.raceLimit) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.raceLimit invalid format!\n", strCode), true, true)
							break
						end
						...
						for dwIndex,v in ipairs(c.raceLimit) do
							if type(v) ~= 'number' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.raceLimit[%d] invalid format!\n", strCode, dwIndex), true, true)
								break
							end

							if v > 9 or v < -9 then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.raceLimit[%d] out of range!\n", strCode, dwIndex), true, true)
								break
							end
						end
					...
					if c.itemRequire then
						if type(c.itemRequire) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemRequire invalid format!\n", strCode), true, true)
							break
						end
							...
							repeat
								if c.itemRequire[t] then
									if type(c.itemRequire[t]) ~= 'table' then
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemRequire.%s invalid format!\n", strCode, t), true, true)
										bSucc = false
										break
									end
									...
									for dwIndex,part in ipairs(c.itemRequire[t]) do
										if type(part) ~= 'table' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemRequire.%s.[%d] invalid format!\n", strCode, t, dwIndex), true, true)
											break
										end

										if part.grade and type(part.grade) ~= 'number' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemRequire.%s.[%d] grade invalid format!\n", strCode, t, dwIndex), true, true)
											break
										end

										if part.lv and type(part.lv) ~= 'number' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemRequire.%s.[%d] lv invalid format!\n", strCode, t, dwIndex), true, true)
											break
										end

										if part.upgLv and type(part.upgLv) ~= 'number' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemRequire.%s.[%d] upgLv invalid format!\n", strCode, t, dwIndex), true, true)
											break
										end
									end
					...
					if c.itemConsume then
						if type(c.itemConsume) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemConsume invalid format!\n", strCode), true, true)
							break
						end
						...
						for dwIndex,consume in ipairs(c.itemConsume) do
							if type(consume) ~= 'table' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemConsume[%d] invalid format!\n", strCode, dwIndex), true, true)
								break
							end

							if not consume.itemCode or type(consume.itemCode) ~= 'string' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemConsume[%d].itemCode invalid format!\n", strCode, dwIndex), true, true)
								break
							end
							...
							if consume["tableCode"] == -1 then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemConsume[%d].itemCode %s invalid type!\n", strCode, dwIndex, consume.itemCode), true, true)
								break
							end
							...
							if not consume["pBaseFld"] then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemConsume[%d].itemCode %s item not exist!\n", strCode, dwIndex, consume.itemCode), true, true)
								break
							end
							...
							if consume.quantity then
								if type(consume.quantity) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemConsume[%d].quantity invalid format!\n", strCode, dwIndex), true, true)
									break
								end
							end
							...
							if ConsumeItemList[consume.itemCode] then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.itemConsume[%d].itemCode %s duplicate item!\n", strCode, dwIndex, consume.itemCode), true, true)
								break
							end
					...
					if c.haveEffectRequire then
						if type(c.haveEffectRequire) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.haveEffectRequire invalid format!\n", strCode), true, true)
							break
						end
						...
						elseif SERVER_2232 then
							...
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, "Lua. Check init.lua server version setting!", true, true)
							break
						end
						...
						for id,v in pairs(c.haveEffectRequire) do
							if id > MaxHaveEffID or id < 0 then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.haveEffectRequire. Key %d out of range!\n", strCode, id), true, true)
								break
							end

							if type(v) ~= 'function' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.haveEffectRequire[%d] invalid format!\n", strCode, id), true, true)
								break
							end
						end
					...
					if c.costDalant then
						if type(c.costDalant) ~= 'number' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.costDalant invalid format!\n", strCode), true, true)
							break
						end
					end
					...
					if c.costGold then
						if type(c.costGold) ~= 'number' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.costGold invalid format!\n", strCode), true, true)
							break
						end
					end
					...
					if c.costProcessingPoint then
						if type(c.costProcessingPoint) ~= 'number' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.costProcessingPoint invalid format!\n", strCode), true, true)
							break
						end
					end
					...
					if c.costHuntingPoint then
						if type(c.costHuntingPoint) ~= 'number' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.costHuntingPoint invalid format!\n", strCode), true, true)
							break
						end
					end
					...
					if c.costGoldenPoint then
						if type(c.costGoldenPoint) ~= 'number' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.costGoldenPoint invalid format!\n", strCode), true, true)
							break
						end
					end
					...
					if c.costPvPCash then
						if type(c.costPvPCash) ~= 'number' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s conditions.costPvPCash invalid format!\n", strCode), true, true)
							break
						end
					end
				end
				...
				if portal_data.openSchedule then
					if type(portal_data.openSchedule) ~= 'table' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule invalid format!\n", strCode), true, true)
						break
					end
					...
					if sz > 1 then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule only one type allowed at a time!\n", strCode), true, true)
						break
					end
					...
					if portal_data.openSchedule.absolute then
						if type(portal_data.openSchedule.absolute) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.absolute invalid format!\n", strCode), true, true)
							break
						end
						...
						for k,v in ipairs(TimeInterval) do
							if schedule[v] then
								if type(schedule[v]) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.absolute.%s invalid format!\n", strCode, v), true, true)
									break
								end

								if schedule[v] < LimitMin[k] or schedule[v] > LimitMax[k] then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.absolute.%s out of range!\n", strCode, v), true, true)
									break
								end
							else
								bSucc = false
					...
					if portal_data.openSchedule.after then
						if type(portal_data.openSchedule.after) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.after invalid format!\n", strCode), true, true)
							break
						end
						...
						for _,v in ipairs(TimeInterval) do
							if schedule[v] then
								if type(schedule[v]) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.after.%s invalid format!\n", strCode, v), true, true)
									break
								end

								if schedule[v] < 0 then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.after.%s out of range!\n", strCode, v), true, true)
									break
								end
							end
						end
					...
					if portal_data.openSchedule.every then
						if type(portal_data.openSchedule.every) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every invalid format!\n", strCode), true, true)
							break
						end
						...
						if sz > 1 then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every only one type allowed at a time!\n", strCode), true, true)
							break
						end
						...
						if schedule.dayOfMonth then
							if type(schedule.dayOfMonth) ~= 'table' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth invalid format!\n", strCode), true, true)
								break
							end
							...
							for dwIndex,interval in ipairs(schedule.dayOfMonth) do
								if type(interval) ~= 'table' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d] invalid format!\n", strCode, dwIndex), true, true)
									break
								end

								if not interval.val or type(interval.val) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d].val invalid format!\n", strCode, dwIndex), true, true)
									break
								end

								if interval.val < 1 or interval.val > 31 then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d].val out of range!\n", strCode, dwIndex), true, true)
									break
								end
								...
								if interval.offset then
									if type(interval.offset) ~= 'table' then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d].offset invalid format!\n", strCode, dwIndex), true, true)
										break
									end
									...
									if interval.offset.hour then
										if type(interval.offset.hour) ~= 'number' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d].offset.hour invalid format!\n", strCode, dwIndex), true, true)
											break
										end

										if interval.offset.hour < 0 or interval.offset.hour > 23 then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d].offset.hour out of range!\n", strCode, dwIndex), true, true)
											break
										end
									end
									...
									if interval.offset.minute then
										if type(interval.offset.minute) ~= 'number' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d].offset.minute invalid format!\n", strCode, dwIndex), true, true)
											break
										end

										if interval.offset.minute < 0 or interval.offset.minute > 59 then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfMonth[%d].offset.minute out of range!\n", strCode, dwIndex), true, true)
											break
										end
									end
								end
						...
						if schedule.dayOfWeek then
							if type(schedule.dayOfWeek) ~= 'table' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek invalid format!\n", strCode), true, true)
								break
							end
							...
							for dwIndex,interval in ipairs(schedule.dayOfWeek) do
								if type(interval) ~= 'table' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d] invalid format!\n", strCode, dwIndex), true, true)
									break
								end

								if not interval.val or type(interval.val) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d].val invalid format!\n", strCode, dwIndex), true, true)
									break
								end

								if interval.val < 1 or interval.val > 7 then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d].val out of range!\n", strCode, dwIndex), true, true)
									break
								end
								...
								if interval.offset then
									if type(interval.offset) ~= 'table' then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d].offset invalid format!\n", strCode, dwIndex), true, true)
										break
									end
									...
									if interval.offset.hour then
										if type(interval.offset.hour) ~= 'number' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d].offset.hour invalid format!\n", strCode, dwIndex), true, true)
											break
										end

										if interval.offset.hour < 0 or interval.offset.hour > 23 then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d].offset.hour out of range!\n", strCode, dwIndex), true, true)
											break
										end
									end
									...
									if interval.offset.minute then
										if type(interval.offset.minute) ~= 'number' then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d].offset.minute invalid format!\n", strCode, dwIndex), true, true)
											break
										end

										if interval.offset.minute < 0 or interval.offset.minute > 59 then
											bSucc = false
											Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.dayOfWeek[%d].offset.minute out of range!\n", strCode, dwIndex), true, true)
											break
										end
									end
								end
						...
						if schedule.day then
							if type(schedule.day) ~= 'table' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.day invalid format!\n", strCode), true, true)
								break
							end
							...
							for dwIndex,interval in ipairs(schedule.day) do
								if type(interval) ~= 'table' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.day[%d] invalid format!\n", strCode, dwIndex), true, true)
									break
								end

								if not interval.hour or type(interval.hour) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.day[%d].hour invalid format!\n", strCode, dwIndex), true, true)
									break
								end

								if interval.hour < 0 or interval.hour > 23 then
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.day[%d].hour out of range!\n", strCode, dwIndex), true, true)
									bSucc = false
									break
								end
								...
								if interval.minute then
									if type(interval.minute) ~= 'number' then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.day[%d].minute invalid format!\n", strCode, dwIndex), true, true)
										break
									end
						...
						if schedule.hour then
							if type(schedule.hour) ~= 'table' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour invalid format!\n", strCode), true, true)
								break
							end

							if not schedule.hour.val or type(schedule.hour.val) ~= 'number' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour.val invalid format!\n", strCode), true, true)
								break
							end

							if schedule.hour.val < 1 or (24 % schedule.hour.val) ~= 0 then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour.val out of range!\n", strCode), true, true)
								break
							end
							...
							if schedule.hour.offset then
								if type(schedule.hour.offset) ~= 'table' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour.offset invalid format!\n", strCode), true, true)
									break
								end
								...
								if schedule.hour.offset.hour then
									if type(schedule.hour.offset.hour) ~= 'number' then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour.offset.hour invalid format!\n", strCode), true, true)
										break
									end

									if schedule.hour.offset.hour < 0 or schedule.hour.offset.hour >= schedule.hour.val then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour.offset.hour out of range!\n", strCode), true, true)
										break
									end
								end
								...
								if schedule.hour.offset.minute then
									if type(schedule.hour.offset.minute) ~= 'number' then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour.offset.minute invalid format!\n", strCode), true, true)
										break
									end

									if schedule.hour.offset.minute < 0 or schedule.hour.offset.minute > 59 then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.hour.offset.minute out of range!\n", strCode), true, true)
										break
									end
								end
							end
						...
						if schedule.minute then
							if type(schedule.minute) ~= 'table' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.minute invalid format!\n", strCode), true, true)
								break
							end

							if not schedule.minute.val or type(schedule.minute.val) ~= 'number' then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.minute.val invalid format!\n", strCode), true, true)
								break
							end

							if schedule.minute.val < 1 or (60 % schedule.minute.val) ~= 0 then
								bSucc = false
								Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.minute.val out of range!\n", strCode), true, true)
								break
							end
							...
							if schedule.minute.offset then
								if type(schedule.minute.offset) ~= 'table' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.minute.offset invalid format!\n", strCode), true, true)
									break
								end
								...
								if schedule.minute.offset.minute then
									if type(schedule.minute.offset.minute) ~= 'number' then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.minute.offset.minute invalid format!\n", strCode), true, true)
										break
									end
									...
									if schedule.minute.offset.minute < 0 or schedule.minute.offset.minute >= schedule.minute.val then
										bSucc = false
										Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s openSchedule.every.minute.offset.minute out of range!\n", strCode), true, true)
										break
									end
								end
							end
				...
				if portal_data.closeSchedule then
					if type(portal_data.closeSchedule) ~= 'table' then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule invalid format!\n", strCode), true, true)
						break
					end
					...
					if sz > 1 then
						bSucc = false
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule only one type allowed at a time!\n", strCode), true, true)
						break
					end
					...
					if portal_data.closeSchedule.absolute then
						if type(portal_data.closeSchedule.absolute) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule.absolute invalid format!\n", strCode), true, true)
							break
						end
						...
						for k,v in ipairs(TimeInterval) do
							if schedule[v] then
								if type(schedule[v]) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule.absolute.%s invalid format!\n", strCode, TimeInterval[k]), true, true)
									break
								end

								if schedule[v] < LimitMin[k] or schedule[v] > LimitMax[k] then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule.absolute.%s out of range!\n", strCode, TimeInterval[k]), true, true)
									break
								end
							else
								bSucc = false
					...
					if portal_data.closeSchedule.after then
						if type(portal_data.closeSchedule.after) ~= 'table' then
							bSucc = false
							Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule.after invalid format!\n", strCode), true, true)
							break
						end
						...
						for _,v in ipairs(TimeInterval) do
							if schedule[v] then
								if type(schedule[v]) ~= 'number' then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule.after.%s invalid format!\n", strCode, v), true, true)
									break
								end

								if schedule[v] < 0 then
									bSucc = false
									Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Portal record:%s closeSchedule.after.%s out of range!\n", strCode, v), true, true)
									break
								end
							end
						end
		...
		if not bSucc then
			TmpDynamicPortals = nil
			Sirin.WriteA(self.pszModReturnGateLogPath, "SirinRiftMgr:validateScriptData(...) bSucc == false!\n", true, true)
		end
...
function SirinRiftMgr:onLoop()
	...
	for id, Gate in pairs(self.m_Gates) do
		repeat
			if Gate.m_bDelete or Gate.m_bOpen then
				break
			end

			if Gate.m_tNextOpenTime >= 0 and Gate.m_tNextOpenTime < self.m_tLoopTime then
				if not Gate.m_LuaScript.probability or Gate.m_LuaScript.probability < 0 or math.random(0, 99) < Gate.m_LuaScript.probability then
					...
					if self:IsDebugLog() then
						...
						Sirin.WriteA(self.pszModReturnGateLogPath, string.format("Rift '%s' open fail (random). Next open: %s\n", Gate.m_strScriptID, szBufr), true, true)
					end
				end
			end
...
function SirinRiftMgr:saveState()
	...
	for _,v in pairs(self.m_ListGates) do
...
function SirinRiftMgr:uninit()
	self:EnableSaveState(false)

	for _,Gate in pairs(self.m_Gates) do
		if Gate:IsOpen() then
			Gate:Close()
		end

		self.m_Gates[Gate.m_strScriptID] = nil
	end

	self:EnableSaveState(true)

	if TmpDynamicPortals then
		ScriptDynamicPortals = TmpDynamicPortals
		TmpDynamicPortals = nil
	end
end
...
function SirinRiftMgr:init()
	...
	if type(ScriptDynamicPortals) ~= 'table' then
		return
	end

	for k,rg in pairs(ScriptDynamicPortals) do
		if rg.portalType == 0 then
			...
			local fnCheckState = function()
				NewGate.m_bOpen = Sirin.GetPrivateProfileIntA(NewGate.m_strScriptID, "IsOpen", 0, self.pszModReturnGateConfigPath) ~= 0 and true or false
				NewGate.m_nUseLeft = Sirin.GetPrivateProfileIntA(NewGate.m_strScriptID, "UseLeft", -1, self.pszModReturnGateConfigPath)
				local _, szValue = Sirin.GetPrivateProfileStringA(NewGate.m_strScriptID, "CloseTime", "-1", self.pszModReturnGateConfigPath)
				...
				if NewGate.m_bOpen then -- check last state
					if tCurTime > NewGate.m_tNextCloseTime then -- check last state expired
						...
						Sirin.WritePrivateProfileStringA(NewGate.m_strScriptID, "IsOpen", "0", self.pszModReturnGateConfigPath)
			...
			if NewGate.m_bOpen then
				...
				if s and s[0] then
					...
					self:addRift(NewGate.m_pCObj, NewGate)
					...
					if s[1] then
						repeat
							...
							self:addRift(NewGate.m_ExitDummyGate.m_pCObj, NewGate.m_ExitDummyGate)
			...
			self.m_Gates[NewGate.m_strScriptID] = NewGate
		end
	end
end
...
function SirinRiftMgr:canUseRift(pLuaRift, pPlayer)
	...
	repeat
		...
		if type(c.patriarchGroupLimit) == 'table' and #c.patriarchGroupLimit > 0 then
			local byPatriarchGroup = Sirin.mainThread.CPvpUserAndGuildRankingSystem.Instance():GetBossType(pPlayer:GetObjRace(), pPlayer.m_Param:GetCharSerial())
		...
		if type(c.raceLimit) == 'table' and #c.raceLimit > 0 then
		...
		for _,del in pairs(DeleteList) do
			pPlayer:Emb_DelStorage(STORAGE_POS.inven, del.pCon.m_byStorageIndex, false, true, "Lua. sirinRift.canUseRift(...)")
		end
...
function SirinRiftMgr:onUse(pLuaRift, pPlayer)
	...
	if pLuaRift.m_nUseLeft > 0 then
		...
		Sirin.WritePrivateProfileStringA(pLuaRift.m_strScriptID, "UseLeft", string.format("%d", pLuaRift.m_nUseLeft), self.pszModReturnGateConfigPath)
	end
