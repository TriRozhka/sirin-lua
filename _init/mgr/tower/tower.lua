...
---@field __index table
---@field bSystemTowerAssist boolean
---@field bPlayerTowerAssist boolean
---@field bTowerSelfDefense boolean
SirinTowerMgr = {
	bSystemTowerAssist = true,
	bPlayerTowerAssist = false,
	bTowerSelfDefense = true,
}

function SirinTowerMgr:new(o)
	o = o or {}
	self.__index = self
	return setmetatable(o, self)
end

TowerMgr = SirinTowerMgr:new()
...
function SirinTowerMgr:TestTowerTarget(pTower, pTarget)
	...
	repeat
		...
		if pTower.m_pMasterTwr then -- we are non system tower
			...
			if self.bPlayerTowerAssist and pTarget.m_ObjID.m_byID == ID_CHAR.monster then
			...
			if self.bSystemTowerAssist and pTarget.m_ObjID.m_byID == ID_CHAR.monster then
				local pMonTar = Sirin.mainThread.objectToMonster(pTarget).m_pTargetChar
...
function SirinTowerMgr:SearchNearEnemy(pTower)
	...
	for _,pTestObj in ipairs(ListPlayers) do
		repeat
			if pTestObj.m_bCorpse or pTower:GetObjRace() == pTestObj:GetObjRace() or not self:TestTowerTarget(pTower, pTestObj) then
				break
			end
	...
	if #aroundEnemyPlayers == 0 then
		...
		for _,pTestObj in ipairs(ListObjects) do
			...
			if idChar == ID_CHAR.animus
				...
				then
				if self:TestTowerTarget(pTower, pTestChar) then
			...
			elseif idChar == ID_CHAR.monster then
				...
				if pMonTar and (self.bTowerSelfDefense and IsSameObject(pTower, pMonTar) or ((self.bSystemTowerAssist and not pTower.m_pMasterTwr or self.bPlayerTowerAssist and pTower.m_pMasterTwr) and pMonTar:GetObjRace() == pTower:GetObjRace())) and self:TestTowerTarget(pTower, pTestChar) then
...
function SirinTowerMgr:IsValidTarget(pTower)
	return self:TestTowerTarget(pTower, pTower.m_pTarget)
end